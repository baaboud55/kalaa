...
const ContactForm: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const subject = encodeURIComponent(`Kalaa Partnership/Investment Inquiry from ${formData.name}`);
      const body = encodeURIComponent(`Name: ${formData.name}\nEmail: ${formData.email}\n\nMessage:\n${formData.message}`);
      window.location.href = `mailto:invest@kalaa.sa?subject=${subject}&body=${body}`;
  ...
  return (
    ...
    >
      <h4 className="text-xl font-bold text-white mb-6">{isRTL ? 'استثمر أو شاركنا النجاح' : 'Partner or Invest'}</h4>
